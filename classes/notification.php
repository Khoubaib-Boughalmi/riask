...
class notification{
    ...
    public function num_notification($user_logged_in){
		$query_notification=mysqli_query($this->con,"SELECT * FROM notifications WHERE user_to='$user_logged_in' and opened ='no' ORDER BY id DESC ");
        $num_notification=mysqli_num_rows($query_notification);
        return $num_notification;
    }


    public function insert_notification($post_id, $user_to, $type,$title) {
		...
		if($type=='comment') {
			...
			$insert_comment_query = mysqli_query($this->con, "INSERT INTO notifications VALUES('','$post_id', '$user_to', '$user_logged_in', '$message','$title', '$link', '$date_time', 'no', 'no','no','yes')");
		}else if($type=='like') {
			...
			$insert_like_query = mysqli_query($this->con, "INSERT INTO notifications VALUES('','$post_id', '$user_to', '$user_logged_in', '$message','$title', '$link', '$date_time', 'no', 'no','yes','no')");
		}
	...
	public function load_notification(){
		$user_logged_in=$this->user_logged_in;
		$query_notification=mysqli_query($this->con,"SELECT *,COUNT(*) as count FROM notifications WHERE user_to='$user_logged_in' group by post_id order by datetime DESC");
		...
		if ($query_notification_num>0) {
			...
			while ($row=mysqli_fetch_array($query_notification)) {
				...
				$user_profile_query=mysqli_query($this->con,"SELECT * FROM users where user_name='$user_to_profile_name'");
				$user_profile_array=mysqli_fetch_array($user_profile_query);
				$user_to_profile_pic=$user_profile_array['profile_pic'];
				...
				$count = (int)$row['count'] - 1;
						if ($row['opened'] == 'yes') {
							...
							if ($count == 0) {
								$notification_value="<div class='dropdown_notification_container dropdown_notification_container_type1_145'><a href='profile.php?user_profile=$user_to_profile_name' id='image_drop_down_profile_link'><img src='$user_to_profile_pic' style='width:3rem;height:3rem;border-radius:10rem;'></a><a href='".$row['link']."&opened_from_notification=true/$title' id='drop_down_link'>".$row['user_from']."reacted to your post :  <br><span>".$row['notification_title']."</span></a></div>";
							}else{
								$notification_value="<div class='dropdown_notification_container dropdown_notification_container_type1_145'><a href='profile.php?user_profile=$user_to_profile_name' id='image_drop_down_profile_link'><img src='$user_to_profile_pic' style='width:3rem;height:3rem;border-radius:10rem;'></a><a href='".$row['link']."&opened_from_notification=true/$title' id='drop_down_link'>".$row['user_from']." and ".$count." other people reacted to your post :  <br><span>".$row['notification_title']."</span></a></div>";
							}
						}else {
							if ($count == 0) {
