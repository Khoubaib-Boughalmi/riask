...
$user_name_loged_in=$_POST['user_name_loged_in'];

include_once('like_dislike.php');
$like_dislike_obj=new likes_dislikes($con,$user_name_loged_in);

$post="";
    $query_load_post=mysqli_query($con,"SELECT * FROM posts where 1 ORDER BY id DESC LIMIT $count" );
	if (mysqli_num_rows($query_load_post)>0) {
            
		while($row=mysqli_fetch_array($query_load_post)){
			if (strstr($row['repored_by'],$user_name_loged_in)==false) {
					...
					<div class='post-body'>
						...
						<div class=' post-body-text'>
							...
							<div class='dropdown_post'>
								...
								if ($row['added_by']==$user_name_loged_in) {
...
var user_name_logged_in = '<?php echo $user_name_loged_in ?>';
var full_dislike_id = $(this).attr('id');

var dislike_id = full_dislike_id.slice(7, full_dislike_id.len);

$.post("dislike_clicked.php", {
	dislike_id_val: dislike_id,
	user_name_logged_in_val: user_name_logged_in
}, function (data) {
	$('.likes_and_bottom_post' + dislike_id).html(data);
})
